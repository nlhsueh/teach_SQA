...
### Java rules
PMD（Programming Mistake Detector）是一款靜態程式碼分析工具，用於檢測原始程式碼中的潛在問題。它支持多種語言，其中 Java 規則最為豐富。PMD 提供了一組預定義的規則集，可幫助開發者維護高質量的程式碼，主要涵蓋以下幾個方面：
