...
靜態檢視（Static Inspection）是在程式碼未被編譯執行前，早期掃除規格漏洞、架構壞味道與資安缺陷的最經濟手段。本章系統性建立從需求、設計到程式碼的全生命週期檢視體系：

```
Ch04 知識架構全景：
【靜態哲學】4.1 軟體檢視概念（冰山比喻 ＆ 靜態 vs 動態互補）
【標準流程】4.2 檢視方法（IEEE 1028、Fagan 流程、角色分工 ＆ AI 現代實踐）
【前期把關】4.3 規格檢視（SRS 7 大撰寫準則 ＆ 雙向追溯）
【架構審查】4.4 設計檢視（DFD/UML 模型檢核 ＆ AI 威脅建模）
【程式碼把關】4.5 程式碼檢視（團隊 Clean Code 清單、Code Smells ＆ OWASP Top 10）
【量化成效】4.6 檢視成效評估（缺失偵測率 ＆ 成本效益度量）
```

| 章節單元 | 核心學習重點 (Key Takeaways) |
| :--- | :--- |
| **[4.1 基本概念](#41-基本概念)** | 理解軟體測試的「冰山比喻」；掌握靜態檢視如何以極低成本早期發現規格與程式碼異常；建立檢視的前置條件。 |
| **[4.2 檢視方法](#42-檢視方法)** | 掌握 IEEE 1028 五種審查類型；精熟 Fagan 檢視 6 大標準步驟與 5 大角色分工；掌握 **AI 輔助檢視（AI PR 機器人先行 + 人類專家把關）** 典範轉移。 |
| **[4.3 規格檢視](#43-規格檢視)** | 掌握 SRS 軟體需求規格書核心品質屬性（正確、無歧義、完整、可驗證、雙向追溯）；精熟規格撰寫 7 大實用準則。 |
| **[4.4 設計檢視](#44-設計檢視)** | 掌握系統架構、介面契約與資料庫 Schema 審查；掌握 DFD / UML 模型檢核清單與 AI 威脅建模分析。 |
| **[4.5 程式碼檢視](#45-程式碼檢視)** | 呼應 Ch02 Clean Code，落實 **團隊 Code Review 檢核清單**；識別 Martin Fowler 22 種 **程式碼臭味 (Code Smells)** 與重構手法；掌握 **OWASP Top 10 核心漏洞防護** 與 PMD 靜態分析。 |
| **[4.6 檢視的評估](#46-檢視的評估)** | 建立檢視量化度量指標：缺失偵測效率 (EDE)、成本效用比 (Cost-Effectiveness)、錯誤密度與缺陷移除率 (DRL)。 |
...
> 🔗 **與第二章的連結與深化**：  
> 在 [**第二章（Ch02 2.2 整潔程式碼 Clean Code）**](ch02_bug.md#22-整潔程式碼-clean-code) 中，我們從**「開發者個人自律」**的視角探討了高品質程式碼的核心心法（意圖自明命名、小巧專注函式、衛語句提早回傳、程式碼即文檔與避免 null）。  
> 
> 來到本章（Ch04），我們將 Clean Code 的準則進一步**昇華為「團隊靜態檢視（Team Code Inspection & Peer Review）的制度化查核標準」**。透過同儕審查、結構化檢核表（Checklists）與靜態分析工具（如 PMD / SonarQube），團隊能在程式碼被編譯執行之前，系統化地掃除各類潛在缺陷、消除架構壞味道（Code Smells），並築起嚴密的安全防線。
...
### 4.5.1 團隊 Clean Code 審查查核清單 (Team Code Review Checklist)

將 Ch02 的 Clean Code 原則落實到團隊 Review 的具體檢核維度：

| 檢核維度 | 審查重點 (Review Questions) | 違反時的常見徵兆 |
| :--- | :--- | :--- |
| **1. 命名自明性** | 變數與函式名稱是否能在不看實作的情況下精確表達業務意圖？ | 出現神祕縮寫（如 `d`, `temp1`）、魔術數字（`86400`）、名不符實 |
| **2. 函式單一職責** | 函式是否只做一件事 (Do One Thing Well)？長度是否控制在 20 行內？ | 超長方法（Long Method）、同一個函式混雜了驗證、計算與資料庫存取 |
| **3. 控制流程複雜度** | 是否運用**衛語句 (Guard Clauses)** 提早回傳？巢狀是否不超過 2 層？ | 深層 `if-else` 階梯（Arrow Anti-Pattern）、過多旗標變數 (Flag Variables) |
| **4. 註解有效性** | 註解是否只解釋「為什麼 (Why)」而非重複程式碼「做了什麼 (What)」？ | 用冗長註解粉飾難懂程式碼、存在被註解掉的廢棄程式碼 (Zombie Code) |
| **5. 健壯防禦性** | 是否使用 Exception 取代錯誤碼？是否杜絕傳遞與回傳 `null`？ | 未攔截的 NPE 風險、空 catch 區塊吞掉例外（Swallowing Exception） |
| **6. 測試保護力** | 提交的變更是否附帶對應的單元測試？邊界案例是否被測試覆蓋？ | 「無測試保護的裸奔程式碼」，導致 Reviewer 無法確認是否破壞既有行為 |
...
### 4.5.2 程式臭味檢視 (Code Smells & Refactoring)

![Code Smells and Refactoring](../../img/ch04/code_smells.jpg)

> 💡 **Clean Code 與 Code Smells 的關係**：  
> * **Clean Code 是目標境界**：程式碼清晰、易讀、高內聚、低耦合。
> * **Code Smells（程式臭味）是病徵警訊**：由 Martin Fowler 等大師提出，代表程式碼結構已經偏離 Clean Code 原則，預示著潛在的維護危機。
> * **Refactoring（重構）是處方藥**：在不改變對外行為的前提下，逐步消除臭味、回歸 Clean Code。

程式碼審查時針對常見程式臭味進行檢驗與重構改善：
