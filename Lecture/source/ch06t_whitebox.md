...
白箱測試是在已知程式內部邏輯與結構的前提下進行的測試，核心目的在於精確度量「程式碼被測試與驗證的深度與強度」。
...
白箱測試（White-Box Testing）是程式碼層級的顯微鏡。本章系統性建立從控制流分析、航空級 MC/DC 覆蓋，到現代變異測試 (PITest) 與 AI 輔助測資生成的完整白箱體系：

```
Ch06 知識架構全景：
【插樁工具】6.1 JaCoCo 涵蓋度量測實戰 ＆ Bytecode 動態插樁原理
【邏輯覆蓋】6.2 涵蓋度階梯 (敘述 ➔ 分支 ➔ 條件 ➔ 多重條件 ➔ MC/DC 航太標準)
【結構分析】6.3 基本路徑測試 (Basis Path) ＆ McCabe 圈複雜度 (Cyclomatic Complexity)
【質量驗證】6.4 變異測試 (Mutation Testing with PITest) ＆ 變異分數
【現代前沿】6.5 AI 輔助白箱測試（未覆蓋路徑測資合成 ＆ 等價變異體識別）
```

| 章節單元 | 核心學習重點 (Key Takeaways) |
| :--- | :--- |
| **[6.1 Lab: 測試涵蓋度](#-61-lab-測試涵蓋度)** | 掌握 JaCoCo 工具與 Java Agent 動態位元組碼插樁（On-the-fly Bytecode Instrumentation）原理。 |
| **[6.2 測試涵蓋度 (Coverage Criteria)](#62-測試涵蓋度-testing-coverage)** | 深入理解涵蓋度階梯：敘述覆蓋 (Statement)、分支覆蓋 (Branch)、條件覆蓋 (Condition)、**MC/DC (DO-178C 航太安全關鍵標準)**；破解「100% 覆蓋率等於高品質」的迷思。 |
| **[6.3 基本路徑測試 (Basis Path Testing)](#63-基本路徑測試-basis-path-testing)** | 繪製控制流程圖 (CFG)；計算 **McCabe 圈複雜度 $V(G) = E - N + 2$**；找出線性獨立基本路徑集合並設計測資。 |
| **[6.4 變異測試 (Mutation Testing)](#64-變異測試-mutation-testing)** | 「測試你的測試」：掌握變異算子 (Mutators)、計算變異得分 (Mutation Score)，並辨析**等價變異體 (Equivalent Mutants)**。 |
| **[6.5 AI 輔助白箱測試與生成](#65-ai-輔助白箱測試與單元測試生成)** | 掌握利用 LLM 針對 JaCoCo 遺漏分支精準逆向合成測資；輔助判讀等價變異體；建立人機協同單元測試 SOP。 |
| **[6.6 練習 (Exercises)](#66-練習-exercises)** | 實戰練習：二分搜尋法、三角形判斷、BMI 計算等多維度涵蓋度與路徑分析。 |
...
也就是 a-c-d 這個路徑並沒有走過。如果我們要走過所有路徑的話，我們需要 2² = 4 個路徑，這看起來不難，但如果有迴圈的話，假設走 20 遍，我們就需要 4²⁰ = 2⁴⁰ ≈ 1.0995 × 10¹²，這已經失去測試的意義了。即便 a-c-d 這個路徑有其意義（或許 c 的動作可能會影響 d 的行為），但 all-path 的測試成本實在太高了。
...
4.  **獨立影響（獨立性）(Condition Independence)：** 對於決策中的每個條件 Cᵢ，必須找到一對測試案例：
      * 在這對案例中，Cᵢ 的值發生變化（T → F 或 F → T）。
      * **所有其他條件的值保持不變。**
      * 由於 Cᵢ 的變化，**整個決策的最終結果也發生了改變。**
...
- 產生程式 (P) 的變異群，Pᵐ = {P₁, P₂, Pᵢ, ... Pₙ};
- T 為測試資料集合，T = {t₁, t₂, ... tₘ};
- 用 T 來測試 P 及 Pᵐ 內的每個程式，當 ∃ tᵢ ∈ T, P(tᵢ) ≠ Pⱼ(tᵢ) 時，表示 tᵢ 可以識別出兩者的差異，此時 Pⱼ 被移除，稱之為被刪除變異體（killed mutant），沒有被移除的稱之為存活變異體（alive mutant）。
- 如果剩下的 存活變異體 不是等價變異，則增加測試案例。
- 回到步驟 3，直到 存活變異體 為空。

> [!NOTE]
> 🏈 考慮以下的程式 *P*。若我們有兩個變異 P₁ 是把 && 改成 ||， P₂ 是把 == 改成 <=，請問 test case (x=10, y=10) 能夠 kill 哪些變異？其所代表的涵意為何？
>
> ```java
> read(X, Y)
> if ((X>=10) && (Y==1))
...
測試案例 (10, 10) 對原來 P 的測試 P(10, 10) 產出的結果為 X = 2，P₁(10, 10) 為 X = 1，輸出的結果不同了，表示此測試案例能夠偵測出不同，故刪除（kill）P₁。P₂(10, 10) 結果仍然為 X = 2 保持不變，無法刪除 P₂。因此我們必須再加入測試案例。

例如我們加入 (10,1) 的測試案例，P(10, 1) 為 1, P₂(10, 1) 為10, 兩者不同，這時候我們可以刪除 P₂, 因為所有的變異都被移除，所以測試案例 OK。
...
等價變異體的識別在計算機科學中是不可判定（Undecidable）的經典難題。然而，AI 能夠透過語意分析與程式碼對比，以極高的準確率預測某個變異體是否為等價變異體。
* **做法**：將原始程式碼與變異體程式碼同時提供給 AI，詢問其「在所有可能的輸入下，這兩個程式碼片段的行為與輸出是否完全等價？」。
