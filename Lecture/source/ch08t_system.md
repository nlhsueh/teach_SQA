...
系統測試（System Testing）是對完整整合之軟硬體系統進行的端到端全面評估，確保軟體在真實業務情境、高併發負載與惡意攻擊下，依然能展現卓越的可靠性與商業價值。
...
系統測試是交付前的終極大考。本章跨越功能與非功能測試，系統性建立涵蓋業務流程、行為驅動 (BDD)、高併發壓測、資訊安全與混沌彈性的全方位測試架構：
...
【攻防安全】8.7 安全性測試 (Security: SAST/DAST ＆ 滲透測試)
【混沌自癒】8.8 回復性與彈性工程 (Resilience: RTO/RPO ＆ 混沌工程)
```
...
| **[8.7 安全性測試 (Security Testing)](#87-安全性測試-security-testing)** | 切換至「攻擊者思維」；掌握靜態白箱 (SAST)、動態黑箱 (DAST)、互動式 (IAST) 掃描與滲透測試標準程序。 |
| **[8.8 回復性與彈性測試 (Resilience)](#88-回復性與彈性測試-recoverability--resilience-testing)** | 建立容錯關鍵指標（**RTO、RPO、MTBF、MTTR**）；實施故障注入 (Fault Injection) 與混沌工程 (Chaos Engineering)。 |
| **[8.9 練習與討論](#89-練習與討論)** | 綜合演練：ATM 結帳流程、BDD 劇本設計、高併發壓測瓶頸分析與安全弱點排查。 |
