...
```
Ch05 知識架構全景：
【自動化基石】5.1 JUnit 5 實戰 ＆ 斷言機制
【輸入域劃分】5.2 邊界值分析 (BVA: 4n+1, 6n+1) ＆ 5.3 等價類分割 (EP: 弱/強涵蓋)
【組合爆炸解法】5.4 全成對測試 (Pairwise / All-Pairs) ＆ 5.5 案例優化
【複雜業務邏輯】5.6 決策表測試 (Decision Table) ＆ 規則化簡
【事件驅動架構】5.7 狀態轉換測試 (State Transition / 1-Switch)
【現代前沿測試】5.8 屬性基礎測試 (Property-Based Testing with jqwik)
```

| 章節單元 | 核心學習重點 (Key Takeaways) |
| :--- | :--- |
| **[5.1 Lab: JUnit](#-51-lab-junit)** | 掌握 JUnit 5 單元測試基礎與斷言機制（`assertEquals` 內容相等 vs. `assertSame` 記憶體同一性）。 |
| **[5.2 邊界測試 (BVA)](#52-邊界測試)** | 掌握「錯誤隱藏在角落」的核心理念；精熟**獨立型 vs. 非獨立型**、**標準邊界 (4n+1) vs. 強固邊界 (6n+1 / 7n+1)** 測試案例設計。 |
| **[5.3 等價分割測試 (EP)](#53-等價分割測試)** | 掌握有效等價類與無效等價類劃分；精熟**弱一般/弱強固 vs. 強一般/強強固等價測試**之選取準則。 |
| **[5.4 全成對組合測試 (Pairwise)](#54-全成對組合測試)** | 解決多參數輸入的「組合爆炸 (Combinatorial Explosion)」難題；掌握 **All-Pairs / 正交表** 原理與工具（PICT / ACTS）。 |
| **[5.5 測試案例的優化](#55-測試案例的優化)** | 學習在覆蓋率不降低的前提下消除冗餘案例，達到測試集精實化。 |
| **[5.6 決策表測試 (Decision Table)](#56-決策表測試)** | 針對複雜業務邏輯，掌握**條件樁 (Condition Stub)、動作樁 (Action Stub)**、規則展開與布林邏輯**規則化簡**。 |
| **[5.7 狀態測試 (State Testing)](#57-狀態測試)** | 針對物件生命週期與狀態機（FSM），設計狀態轉換矩陣與 **0-Switch / 1-Switch 覆蓋路徑**。 |
| **[5.8 屬性基礎測試 (PBT)](#58-屬性基礎測試-property-based-testing)** | 掌握 SQA 2.0 現代前沿技術：使用 **jqwik** 定義核心數學不變量，自動生成海量隨機測資並執行**測資收縮 (Shrinking)**。 |
| **[5.9 實戰練習題](#️-練習)** | 涵蓋游泳池計費、青年成家貸款、電子表狀態機與三角形多維案例實戰。 |
...
**正確答案：X (False)**
* **解析**：
  * `assertSame(a, b)` 驗證的是**參照同一性 (Reference Equality)**，即比對記憶體地址是否相同（`a == b`）。
...
非獨立型一般邊界測試又稱為 *最差情況邊界測試*（worst case boundary value testing）(真倒楣，兩個錯誤同時在邊界時產生)。這種情況，我們就必須用「乘積」的方式來設計測試案例。每一個變數有 min, min+, norm, max-, max 等五種狀況，所以共需要 5ⁿ 個測試案例。這是一個不小的數目，如果有五個變數，則需要 5⁵ = 3,125 個測試案例。表 X 表示三角形程式在非獨立一般邊界測試時的狀況，因為是做乘積，總共會有 `5*5*5=125` 個測試案例。
...
同上，這次我們考慮強固的方案，亦即例外的狀況 max+, min- 考慮進去，所以共需要 7ⁿ 個測試案例，因為每個變數會有以下 7 個資料：
...
當然，這樣的情況測試案例就會更多，以五個變數而言，供需要 7⁵ = 16,807 個測試案例。
...
**正確答案：B**
* **解析**：
  * 每個變數有 6 個邊界點 (`min-`, `min`, `min+`, `max-`, `max`, `max+`)，加上所有變數都取正常值 (`norm`) 的 1 個中心案例。
...
### 簡易等價分割
簡易等價分割依據系統規格所制定或描述的「範圍」，簡易的將一個變數分割成數個正常與數個異常的分割，而不對針對工程師可能犯的錯誤或程式架構進行猜測進而設計測試案例。例如上述的例子，exam可以有三個分割：e⁻, e, e⁺ 分表代表 <0, 0-60, >60。hw 可以有三個分割：h⁻, h, h⁺ 分別代表 <0, 0-40, >40 等。但這樣單純以輸入變數來分割很容易可以想像是不夠的：因為我們要檢驗分數是否能夠正確的落在 A, B, C, D 區塊中。所以把輸出變數分割也考慮進來：g⁻, g⁶, g⁷, g⁸,g⁹, g⁺  分為 < 0, 0–69, 70–79, 80–89, 90–100, > 100等。
...
| x,y   | e⁻ | e | e⁺ | e! | h⁻ | h | h⁺ | h! | g⁻ | g⁶ | g⁷ | g⁸ | g⁹ | g⁺ | Output |
| ----- | ----- | --- | ----- | ----- | ----- | --- | ----- | ----- | ----- | ----- | ----- | ----- | ----- | ----- | ------ |
| -5,-5 | v     |     |       |       | v     |     |       |       | v     |       |       |       |       |       | !      |
| 30,35 |       | v   |       |       |       | v   |       |       |       | v     |       |       |       |       | A      |
| 30,45 |       | v   |       |       |       | v   |       |       |       |       | v     |       |       |       | B      |
...
總結來說，我們將陣列元素分為一個元素（a¹）、多個元素（a*）與 0 個元素（a⁰）。結果可能有找到（fᵗ）或沒有找到（fᶠ），輸出的 key 元素可能是在陣列的第一個位置（c¹）、中間位置（cᵐ）或是最後的位置（cˡ）。k! 表示輸入的 key 值不符合要求的型態或範圍，! 表示系統會做例外處理。等價分割如下：

 
- R₁: 陣列只有一個元素，有找到
- R₂: 陣列只有一個元素，沒找到
- R₃: 陣列有多個元素，有找到，在第一個位置
- R₄: 陣列有多個元素，有找到，在中間位置
- R₅: 陣列有多個元素，有找到，在最後位置
- R₆: 陣列有多個元素，沒找到
- R₇: 陣列沒有元素，沒找到
...
|       | K   | Array  | k | k! | a¹ | a* | a⁰ | fᵗ | fᶠ | c¹ | cᵐ | cˡ | ! |
| ----- | --- | ------ | --- | ----- | ----- | ----- | ----- | ----- | ----- | ----- | ----- | ----- | --- |
| R₁ | 7   | 7      | v   |       | v     |       |       | v     |       | v     |       |       |     |
| R₂ | 7   | 8      | v   |       | v     |       |       |       | v     |       |       |       |     |
| R₃ | 7   | 7,9,11 | v   |       |       | v     |       | v     |       | v     |       |       |     |
...
- M₁= {month | month has 30 days}
- M₂= {month | month has 31 days}
- M₃= {month | month is February}
- D₁= {day | 1 ≤ day ≤ 28}
- D₂= {day | day = 29}
- D₃= {day | day = 30}
- D₄= {day | day=31}
- Y₁= {year | year = 1900}
- Y₂= {year | 1812 ≤ year ≤ 2012 AND year ≠ 1900 AND (0 = year mod 4}
- Y₃= {year | 1812 ≤ year ≤ 2012 AND 0 ≠ year mod 4}
...
**正確答案：B**
* **解析**：
  * **弱等價**基於「單一錯誤假設」，目標是讓每個等價類（Class）的代表值在所有測試案例中至少被涵蓋一次。
...
看過前述幾個例子，當我們要執行非獨立測試時，測試案例的成長是很可怕的，例如有 n 個變數，每一個可能有 m 個分割，如果我們每個分割取一個值來做非獨立測試，則有 mⁿ 個測試案例需要測試。這樣近乎窮盡式的測試方法 在成本可能太高。
...
**正確答案：B**
* **解析**：
  * 根據經驗法則與統計研究（例如 NIST 的研究），大約 60% 至 90% 的軟體缺陷可以由單一變數與雙變數交互作用（2-way）觸發。
...
* **具體做法：**
    * **扣除等效的例外類別：** 如果所有因子都有**相同**的異常行為（例如，所有因子的輸入超限都會導致相同的錯誤碼），則您不需要為每個因子測試這個異常的組合。您可以將這些異常類別視為一個**單獨的集合**，只在特定的、最能代表異常的組合中測試，或者只測試它們與有效類別的組合。
...
**正確答案：B**
* **解析**：
  * 正交表 (OATS) 是一種高度結構化的數學工具，要求任意兩列中所有水平組合出現的次數必須「完全相同」（均勻平衡）。
...
在下面三角形的例子中，我們用 C₁ 表示 a, b, c 不為三角形的狀況，這樣 CAR 才不會膨脹的太大，而且可閱讀性也高，如果我們把 C₁ 拆解為 C₁₁: a + b ≥ c, C₁₂: b + c ≥ a, C₁₃: a + c ≥ b，當然也可以，但表會變成很大難以閱讀與分析。
...
| **日期 (Day)** | D1 = {1 ≤ day ≤ 28}<br>D2 = {day = 29}<br>D3 = {day = 30}<br>D4 = {day = 31} |
| **年份 (Year)** | Y1 = {世紀平年: year = 1900}<br>Y2 = {一般閏年: 1812 ≤ year ≤ 2012 且 year ≠ 1900 且 year mod 4 = 0}<br>Y3 = {一般平年: 1812 ≤ year ≤ 2012 且 year mod 4 ≠ 0} |
...
**正確答案：B**
* **解析**：
  * 決策表是表達**「條件組合與動作（輸入組合與輸出行為）之間因果邏輯關係」**的最強工具，非常適合用來分析業務邏輯極其複雜的商務規則。
...
1. **屬性 (Property) / 不變量 (Invariant)**：
   ...
   * *例如排序演算法*：給定任何 List `A`，排序後的 List `B` 必須滿足：
     ...
     * 對於 `B` 中任何相鄰元素 B[i] 與 B[i+1]，都必須滿足 B[i] ≤ B[i+1]。
2. **自動生成測資 (Generator)**：
...
3. 關於邊界測試，以下何者正確？
	...
	- 非獨立強固邊界測試其測試案例個數最少為 7ⁿ + 1，其中 n 為變數的個數。
