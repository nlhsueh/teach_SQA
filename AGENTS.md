...
## 🚨 繁體中文與台灣軟體工程用語規範 (Strict Terminology Rule)
* **嚴禁使用中國大陸用語「代碼」，一律使用繁體台灣軟體工程用語「程式碼」**：
  * 在所有講義、實習文件、程式碼註解、Commit 訊息、對話回覆中，嚴格落實術語轉換：
    - ❌ 代碼 ➔ ✅ 程式碼 (Code)
    - ❌ 源代碼 / 源碼 ➔ ✅ 原始碼 / 原始程式碼 (Source Code)
    - ❌ 寫代碼 ➔ ✅ 寫程式 / 撰寫程式碼 (Coding / Programming)
    - ❌ 代碼審查 ➔ ✅ 程式碼審查 (Code Review)
    - ❌ 代碼檢視 ➔ ✅ 程式碼檢視 (Code Inspection)
    - ❌ 代碼重構 ➔ ✅ 程式碼重構 (Code Refactoring)
    - ❌ 代碼覆蓋率 ➔ ✅ 程式碼涵蓋率 / 覆蓋率 (Code Coverage)
    - ❌ 代碼補全 ➔ ✅ 程式碼補全 / 程式碼自動完成 (Code Completion)
    - ❌ 錯誤代碼 ➔ ✅ 錯誤碼 (Error Code)
    - ❌ 重複代碼 ➔ ✅ 重複程式碼 (Duplicated Code)
    - ❌ 死代碼 ➔ ✅ 死碼 / 無用程式碼 (Dead Code)
    - ❌ 負載即代碼 ➔ ✅ 負載即程式碼 (Load as Code)
    - ❌ 規格即代碼 / 文件即代碼 ➔ ✅ 規格即程式碼 / 文件即程式碼 (Doc/Spec as Code)
* **其他台灣軟體工程名詞**：函式 (Function)、類別 (Class)、物件 (Object)、陣列 (Array)、介面 (Interface)、執行緒 (Thread)、並發 (Concurrency)、預設 (Default)。
...
* **語言風格**：使用流暢、專業且親切的繁體中文（遵循上述台灣軟體工程用語，如：函式、變數、類別、物件、重構、缺陷、程式碼）。
* **教學結構**：
  * **金句與寓言**：章節開頭引用大師名言或生動寓言（如溫伯格寓言）。
  * **圖文並茂**：架構圖、狀態機、矩陣圖與流程圖置於 `img/` 目錄中，並附帶詳細「圖形解說」。
  * **程式碼對照**：提供良好（✅）與劣質（❌）程式碼對照範例。
